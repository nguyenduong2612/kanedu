      ...
      <IonContent fullscreen>
        <IonList>
          {ranking.topten.map((user: any, index: number) => {
            return (
              <IonCard id={`rank-${index+1}-card`} mode="ios" key={index} routerLink={`/users/${user.uid}`}>
                <IonGrid>
                  <IonRow>
                    <IonCol size="2" className="rank-number__wrapper">
                      <span className="rank-number__number" id={`rank-${index+1}`}>{index+1}</span>
                    </IonCol>
                    <IonCol size="10" className="rank-info__wrapper">
                      <img alt="avatar" className="rank-info__img" src={user.profileURL}></img>
                      <div className="rank-info__name">{user.name}</div>
                      <div className="rank-info__exp"><IonIcon icon={flame} color="primary"></IonIcon>{user.exp}</div>
                    </IonCol>
                  </IonRow>
                </IonGrid>
              </IonCard>
